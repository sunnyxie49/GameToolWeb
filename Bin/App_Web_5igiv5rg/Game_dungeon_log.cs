...
public class Game_dungeon_log : Page, IRequiresSessionState
{
	...
	protected string BindGuildName(object item, string target)
	{
		...
		if ((int)DataBinder.Eval(item, target + "guild_id") > 0)
		{
			...
			string text = link.GuildPopup(a, (int)DataBinder.Eval(item, target + "guild_id"), (string)DataBinder.Eval(item, target + "guild_name"));
			if (num > 0)
			{
				if (a == "program" || a == "devel")
				{
					return string.Format("<img src='{0}{1}' align='absmiddle'> {2}", ConfigurationManager.AppSettings["guild_icon_url_test"], (string)DataBinder.Eval(item, target + "guild_icon"), text);
				}
				return string.Format("<img src='{0}{1}' align='absmiddle'> {2}", ConfigurationManager.AppSettings["guild_icon_url"], (string)DataBinder.Eval(item, target + "guild_icon"), text);
			}
			return $"{text}";
		}
		return "";
