...
public class Game_summon_info : Page, IRequiresSessionState
{
	...
	private void GetData(string server, long item_id)
	{
		...
		sqlCommand.CommandText = "gmtool_GetSummon_info_useSid";
		sqlCommand.CommandType = CommandType.StoredProcedure;
		sqlCommand.Parameters.Clear();
		sqlCommand.Parameters.Add("@summon_id", SqlDbType.BigInt).Value = item_id;
		sqlConnection.Open();
		...
		if (sqlDataReader.Read())
		{
			...
			{
				decimal d = (decimal)sqlDataReader["immoral_point"];
				if (d >= 100m)
				{
					flag = true;
				}
