...
public class Game_AvatarItemMove : Page, IRequiresSessionState
{
	...
	protected string BindModifyForBank(object item)
	{
		...
		{
			return $"<input type='button' class=\"btn btn-primary btn-small\" value='Modify' onClick=\"OpenOwnerChangeItemPopup('{hserver}', 'character','{num.ToString()}')\">";
		}
		return "";
	}
	...
	protected void Button1_Click(object sender, EventArgs e)
	{
		...
		try
		{
			...
			using (GameLogBiz gameLogBiz = new GameLogBiz())
			{
				dataSet = gameLogBiz.GetAvatarList(connectionString, account_id, avatar_id);
			}
