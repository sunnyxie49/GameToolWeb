...
public class Game_character_info_bank : Page, IRequiresSessionState
{
	...
	protected Literal ltrBankGold;

	protected TextBox txtBankGold;

	protected Button btnChangeMoney;

	protected Panel pnlBankGold;
	...
	private void GetCharacterInfo()
	{
		...
		sqlConnection.Open();
		sqlCommand.CommandText = "gmtool_v2_get_character_info_bank";
		...
		{
			ltrBankGold.Text = ((long)dataSet.Tables[2].Rows[0]["cnt"]).ToString("#,##0");
			txtBankGold.Text = ((long)dataSet.Tables[2].Rows[0]["cnt"]).ToString();
			pnlBankGold.Visible = true;
		}
	}

	protected void chkSeeDeletedCharacter_CheckedChanged(object sender, EventArgs e)
	{
	}

	protected void btnChangeMoney_Click(object sender, EventArgs e)
	{
		if (admin.Authority(ddlServer.SelectedValue, "gmtool_change_bank_gold"))
		{
			long result = 0L;
			long.TryParse(txtBankGold.Text, out result);
			string result_msg;
			if (result.ToString() != txtBankGold.Text)
			{
				common.MsgBox(this, "CBank", "Check Bank Money");
			}
			else if (acm.SetBankGold(ddlServer.SelectedValue, character_id, result, out result_msg))
			{
				common.MsgBox(this, "CBank", "Modify Success \n\n" + result_msg);
			}
			else
			{
				common.MsgBox(this, "CBank", "Modify Failure \n\n" + result_msg);
			}
		}
		else
		{
			common.MsgBox(this, "CBank", "[Change Bank Gold] No Permission.");
		}
	}
}
