...
public class Game_Default : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (base.Request.QueryString["desc1"] == "1")
			{
				chkSort1.Checked = true;
			}
			if (base.Request.QueryString["user"] == "1")
			{
			...
			if (base.Request.QueryString["gm"] == "1")
			{
				chkGM.Checked = true;
			}
			else
			...
			if (base.Request.QueryString["game"] == "1")
			{
				chkGameAccount.Checked = true;
			}
			else
			{
				chkGameAccount.Checked = false;
			}
			if (base.Request.QueryString["connect"] == "1")
			{
			...
			if (base.Request.QueryString["create_time"] == "1")
			{
				chkCreateTime.Checked = true;
			}
			else
			{
				chkCreateTime.Checked = false;
			}
			if (base.Request.QueryString["auto_used"] == "1")
			{
			...
			if (base.Request.QueryString["like"] == "1")
			{
	...
	private void GetCharacterList()
	{
		...
		int listSize = 10;
		text3 += $"&pageSize={result}&server={ddlServer.SelectedValue}";
		if (txtAccount.Value != "")
		{
			...
			text3 += $"&account={txtAccount.Value.Trim()}";
		}
		if (txtCharacter.Value != "")
		{
			...
			text3 += $"&name={txtCharacter.Value.Trim()}";
		}
		if (chkUser.Checked)
		{
			text3 += "&user=1";
		...
		{
			text3 += $"&create_time=1&start={txtStart.Text}&end={txtEnd.Text}";
		...
		{
			text3 += "&see_delete_character=True";
		}
		...
		if (ddlSort1.SelectedIndex > 0)
		{
			if (chkSort1.Checked)
			{
				...
				text3 += $"&sort1={ddlSort1.SelectedIndex}&desc1=1";
			}
			else
			{
				text2 += $"order by {ddlSort1.SelectedValue}";
				text3 += $"&sort1={ddlSort1.SelectedIndex}";
			}
		}
		if (ddlJob.SelectedValue != "all")
		{
			text3 += $"&job={ddlJob.SelectedValue}";
			text += " and job = @job";
		}
		if (text2 == "")
		{
		...
		ltrPageTop.Text = paging.PageList(base.Request.Path, text3, totalCnt, ref page, result, listSize);
	...
	protected void Button2_Click(object sender, EventArgs e)
	{
		...
		int totalCnt = (int)sqlCommand.ExecuteScalar();
		ltrTotalCnt.Text = totalCnt.ToString();
		ltrPageTop.Text = common.PageList(base.Request.Path, text3, totalCnt, ref page, result, listSize);
		ltrPageBottom.Text = ltrPageTop.Text;
		StringBuilder stringBuilder = new StringBuilder(2000);
	...
	protected string BindName(object item)
	{
		...
		{
			decimal num4 = (decimal)DataBinder.Eval(item, "immoral_point");
			if (num4 >= 100m)
			{
				flag = true;
			}
