...
public class Game_edit_item : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		{
			common.MsgBox(this, "CEditItem", "您需要服务器值");
		}
		else if (base.Request.QueryString["sid"] == null || base.Request.QueryString["sid"] == "")
		{
			common.MsgBox(this, "CEditItem", "您需要Item sid值");
		}
		else
		{
	...
	private void GetItemInfo()
	{
		...
		{
			ltrItemInfo.Text = "道具代码在ItemResource中不存在";
		}
		else
		{
			...
			if (!string.IsNullOrEmpty(text6) && text6.Length >= 6 && text6[6].ToString() == "1")
			{
				txtCnt.Enabled = true;
			}
	...
	protected void btnChange_Click(object sender, EventArgs e)
	{
		...
		if (txtRemainTime.Enabled)
		{
			...
			{
				DateTime dateTime = DateTime.Parse(txtRemainTime.Text);
				int result2 = 0;
				int.TryParse(ConfigurationManager.AppSettings["GMT"], out result2);
				result = int.Parse((dateTime - new DateTime(1970, 1, 1).AddHours(result2)).TotalSeconds.ToString());
			}
			if (acm.EditItem(server, sid, enhance, level, cnt, @checked, result, socket_, out var result_msg))
			{
				common.MsgBox(this, "CEditItem", "修改 :  成功 \n\n" + result_msg);
			}
			else
			{
				common.MsgBox(this, "CEditItem", "修改 :  失败 \n\n" + result_msg);
			}
		}
	...
	protected void btnPetNameChange_Click(object sender, EventArgs e)
	{
		...
		if (sqlDataReader.Read())
		{
			...
			string text2 = (string)sqlDataReader["result_msg"];
			if (flag)
			{
				common.MsgBox(this, "CEditItem", "修改:  成功 \n\n" + text2);
			}
	...
	protected void btnChange2_Click(object sender, EventArgs e)
	{
		...
		{
			common.MsgBox(this, "CEditItem", "没有权限");
		}
		GetItemInfo();
	}
}
