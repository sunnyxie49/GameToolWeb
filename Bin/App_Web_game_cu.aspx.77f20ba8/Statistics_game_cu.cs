...
public class Statistics_game_cu : Page, IRequiresSessionState
{
	...
	protected void GetData()
	{
		...
		DateTime dateTime = default(DateTime).AddYears(year - 1).AddMonths(month - 1).AddDays(day - 1);
		DateTime dateTime2 = default(DateTime).AddYears(year - 1).AddMonths(month - 1).AddDays(day)
			.AddSeconds(-1.0);
	...
	protected void hdNowDate_Click(string args)
	{
		...
		if (array.Length >= 3)
		{
			month = int.Parse(array[0].ToLower());
			day = int.Parse(array[1].ToLower());
			year = int.Parse(array[2].ToLower());
			GetData();
	...
	{
		string value = hdRadioCheck.Value;
