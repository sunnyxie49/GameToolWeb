...
public class Game_guild_info : Page, IRequiresSessionState
{
	...
	private void GetData()
	{
		...
		if (num > 0)
		{
			...
			{
				ltrAllianceBlockTime.Text = string.Format("- Alliance Block Time :{0}", dateTime.ToString("yyyy-MM-dd HH:mm:ss"));
			}
			else
			{
	...
	protected string BindGuildName(object item)
	{
		...
		if (num > 0)
		{
			string text3 = selected_server;
			if (text3 == "program" || text3 == "devel")
			{
	...
	protected string BindGuildLeader(object item)
	{
		...
		{
			decimal num5 = (decimal)DataBinder.Eval(item, "leader_immoral_point");
			if (num5 >= 100m)
			{
				flag = true;
			}
