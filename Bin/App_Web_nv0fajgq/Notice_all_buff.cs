...
public class Notice_all_buff : Page, IRequiresSessionState
{
	...
	protected void AddEventState(string server_name, int state_id, int state_lv)
	{
		...
		short port;
		string password;
		server.GameServerInfo(server_name, out address, out port, out password);
	...
	protected void GetStateList(string server_name)
	{
		...
		ltrList.Text += "<colgroup><col width='35%' /><col width='35%' /><col width='30%' /></colgroup><thead><tr><th>Server Name</th><th>State ID</th><th>State Lv</th></tr></thead><tbody>";
