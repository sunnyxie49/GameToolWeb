...
public class Game_Skin_skin_item_list : UserControl
{
	...
	private void rptCharacterList_ItemDataBound(object sender, RepeaterItemEventArgs e)
	{
		...
		literal11.Text = string.Format("<input class='btn btn-inverse btn-small' type='button' value='Modify' onClick=\"OpenEditItemPopup('{0}', '{1}')\">", SelectedServer, ((DataRowView)e.Item.DataItem)["sid"]);
		...
		{
			literal11.Text += string.Format("<input class='btn btn-inverse lastBtn btn-small' type='button' value='Summon' onClick=\"OpenSummonPopup('{0}', '{1}')\">", SelectedServer, ((DataRowView)e.Item.DataItem)["sid"]);
		}
