...
public class Statistics_server_down : Page, IRequiresSessionState
{
	...
	private void GetData(string check)
	{
		...
		sb.AppendFormat("<h4>{0}년 {1}월</h4>", check.Substring(0, 4), check.Substring(4, 2));
		sb.Append("<table class='table table-striped table-bordered'><tbody>");
		while (sqlDataReader.Read())
		{
			sb.Append("<tr>");
			DateTime dateTime = (DateTime)sqlDataReader["log_date"];
			string value = string.Format("<td class='redText'>{0}</td><td class='redText'>{1}</td><td class='redText'>#정기점검</td>", dateTime.ToString("yyyy-MM-dd HH:mm:ss"), (string)sqlDataReader["server"]);
	...
	protected string BindDate(object item)
	{
		...
		return $"{text.Substring(0, 4)}년 {text.Substring(4, 2)}월";
	}
